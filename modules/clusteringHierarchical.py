...
def hierarchical_agglomerative(X: np.ndarray,
    ...
    while len(clusters) > 1:
        ...
        if display_steps:
            print(f"Step {len(linkage_rows)}: merge clusters {id_i} & {id_j} | dist={best_dist:.6f} | size={new_size}")
...
def plot_dendrogram_manual(linkage: np.ndarray, labels=None, title="Dendrogram", figsize=(10, 6)):
    """
    Membuat visualisasi dendrogram dari linkage matrix manual.
    """
    if linkage is None or len(linkage) == 0:
        fig, ax = plt.subplots(figsize=figsize)
        ax.text(0.5, 0.5, "Linkage kosong, tidak bisa plot dendrogram.", ha='center', va='center')
        return fig
